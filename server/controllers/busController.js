...
busController.addCustomer = async (req, res, next) => {
  try {
    ...
    const data = [req.body.phone, req.body.name, business];
...
busController.addStar = async (req, res, next) => {
  try {
    ...
    const addingStar =
      "UPDATE business_info SET num_of_visits=num_of_visits + ? WHERE business_name=$1 AND phone=$3";
...
busController.getDash = async (req, res, next) => {
  ...
  try {
    ...
    const busDash = `SELECT a.name AS customer_name, 
      ...
      FROM accounts a 
      LEFT JOIN business_info b ON a.name = b.customer_name AND b.business_name = '${data}' 
      WHERE a.user_type = 'customer' 
      ORDER BY num_of_visits DESC`;
    const result = await pool.query(busDash);
    res.locals.allCustomers = result;
    return next();
  } catch (err) {
    // error handling
    return next({
